...
- 支持多段工作時間設置、按域名修改語義分類、按小時活躍度視圖
- 支持輕量模式、日報 Markdown 導出和多屏截圖策略
...
| 時間範圍識別 | 自動理解“昨天”“本週”“最近 N 天”等自然語言時間範圍 |
| Session 聚合 | 把碎片活動整理為連續工作段，更容易看出完整工作節奏 |
| 待辦提取 | 從訪問頁面、窗口標題和上下文裡提煉可能的後續事項 |
| 日報生成 | 結構化日報，支持歷史回看、按小時活躍度摘要、Markdown 導出和 AI 附加提示詞 |
| 雙模式回答 | 可選基礎模板或 AI 增強，兼顧零配置和表達質量 |
| 網站語義標記 | 在概覽頁點擊瀏覽器域名，可修改其語義分類（如標記為「休息娛樂」則不計入工作時長），修改後自動回填歷史記錄 |
| 多段工作時間 | 支持設置多個工作時段（如上午 + 下午），休息時間不計入工作時長 |
| 桌面化身 Beta（持續完善中） | 邁向個人工作 Agent 的第一步，未來將成為能感知工作上下文、主動提醒和輔助決策的桌面夥伴 |

### Bot 聯動 Beta（Telegram / 飛書）

- 支持透過本地 API + 多設備註冊，從 Telegram / 飛書遠端查詢與生成日報
- 支持命令：`/devices`、`/device`、`/reports`、`/report`、`/generate`（飛書同樣支持中文關鍵詞）
- 僅限個人和本人多設備聯動使用，不得用於員工監控、績效考核或隱形追蹤

### MCP Server Beta

透過 stdio 協議將工作記錄接入 AI 編碼工具，支持查詢時間線、生成日報、分析工作模式等。

- 內建 11 個工具（時間線查詢、日報生成、工作會話分析等）、3 個資源、3 個提示詞模板
- 內建 5 個技能（每日回顧、每週總結、專案時間審計、工作模式分析、專注會話建議），透過 `execute_skill` 觸發
- 帶策略引擎，自動過濾隱私應用和脫敏內容

<details>
<summary>配置方法</summary>

需先從原始碼編譯 MCP Server（後續會隨應用一起發布）：

```bash
cargo build --release -p work-review-mcp-server
# 二進位檔位於 target/release/work-review-mcp-server
```

以下為常見工具的設定參考，JSON 格式基本一致，按各自工具的設定入口新增即可：
...
| 工具 | 設定位置 | 作用域 |
|------|----------|--------|
| Claude Code | `~/.claude/settings.json` | 全域 |
| Cursor | Settings > MCP 或 `.cursor/mcp.json` | 全域 / 專案級 |
| VS Code (Copilot) | `.vscode/mcp.json`（`servers` 欄位，需加 `"type": "stdio"`） | 專案級 |

> 如果不設定環境變數，預設使用系統資料目錄下的資料庫和設定檔。
...
<img src="docs/Introduction_tw/節點.png" alt="Work Review 接入管理" />

接入管理頁用於管理裝置身份、本地 API、Bot 聯動和 MCP Server，方便外部工具對接和多裝置日報生成。
...
| **助手** | 用自然語言直接提問，基於本地記錄回答工作問題 |
| **日報** | 查看、生成和回看任意日期的日報，支持附加提示詞和 Markdown 導出 |
...
- `Windows`：截圖和桌寵聯動預設不需要額外系統隱私授權；安裝時依賴 Microsoft Edge WebView2 Runtime（若下載連線受限，可手動安裝後重試）。
...
<p align="center">
  <img src="docs/group/vx.png" alt="Work Review 微信群 QR code" width="220" />
</p>

<p align="center"><strong>公眾號</strong></p>

<p align="center">
  <img src="docs/group/gzh.jpg" alt="Work Review 公眾號 QR code" width="220" />
</p>
